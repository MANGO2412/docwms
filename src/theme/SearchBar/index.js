...
function useSearchParameters({contextualSearch, ...props}) {
  ...
  // };
   ...
   const {i18n} = useDocusaurusContext(); // Obtiene el contexto de i18n
  const currentLang = i18n.currentLocale; // Idioma actual
  ...
  // Añade el filtro de idioma dinámico
  const langFilter = `lang:${currentLang}`;
  const allFilters = [langFilter, ...configFacetFilters];

  const facetFilters = contextualSearch
    ? mergeFacetFilters(contextualSearchFacetFilters, allFilters)
    : allFilters;
